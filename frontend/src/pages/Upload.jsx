...
export default function Upload() {
  ...
  // 🛡️ Premium Lock State Layout Fallback Template
  if (!isPremium) {
    return (
      <div className="min-h-screen bg-[#030712] text-slate-200 flex flex-col items-center justify-center p-4">
        <div className="p-8 bg-slate-900/60 border border-slate-800 rounded-2xl max-w-md w-full backdrop-blur-xl text-center space-y-4 shadow-2xl">
          <ShieldAlert className="w-12 h-12 text-amber-500 mx-auto animate-pulse" />
          <h3 className="text-xl font-extrabold text-white tracking-tight">Pro Storage Vector Locked</h3>
          <p className="text-sm text-slate-400 leading-relaxed">
            Blending custom business operational rules or internal accounting docs with our foundational regulatory RAG models requires an upgraded active subscription tier.
          </p>
          <button 
            onClick={() => navigate('/settings')}
            className="w-full inline-flex items-center justify-center gap-2 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl text-sm transition-all shadow-lg shadow-indigo-600/30 cursor-pointer"
          >
                  ...
                  <tr key={file.id} className="hover:bg-slate-950/40 transition-colors duration-150">
                    ...
                    <td className="p-4 pr-6 text-right">
                      <button
                        ...
                        className="text-slate-500 hover:text-red-400 p-2 hover:bg-red-950/20 rounded-xl transition-all duration-150 cursor-pointer"
